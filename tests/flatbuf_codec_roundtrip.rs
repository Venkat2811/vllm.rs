...
use vllm_rs::ipc::flatbuf_codec;
use vllm_rs::ipc::myelon_ipc::{MyelonRequest, MyelonResponse};
...
fn roundtrip_request(req: &MyelonRequest) -> MyelonRequest {
    ...
    let bytes = flatbuf_codec::encode_request(req).expect("encode");
    flatbuf_codec::decode_request(kind, &bytes).expect("decode")
}

fn roundtrip_response(resp: &MyelonResponse) -> MyelonResponse {
    let kind = resp.kind() as u8;
    let bytes = flatbuf_codec::encode_response(resp).expect("encode");
    flatbuf_codec::decode_response(kind, &bytes).expect("decode")
}
