#!/usr/bin/env bash
# Mac M3 Max scaling sweep — Qwen3-4B (BF16, ~8 GB).
# Tests whether the typed-mode delta on the 0.6B toy model holds at a
# realistically-sized model where prefill compute is much higher.
set -euo pipefail
...
OUT_DIR="$ROOT/scripts/mac_p2_5_qwen4b_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUT_DIR"
...
run_cell() {
    ...
    "$BIN" --m "$MODEL" --d 0 \
        ...
        --prompts "Tell me about distributed systems and shared memory in detail, with examples and tradeoffs." \
        "${args[@]}" 2>&1 > "$log"
...
for cfg in "${CONFIGS[@]}"; do
    ...
    echo "═══ Qwen3-4B  batch=$BATCH max_tokens=$MAXTOK ═══"
    for run in 1 2; do
        echo "  run $run:"
        echo "    socket:"
...
print()
print(f"{'batch':>5} {'maxtok':>6} {'sock_p':>10} {'typ_p':>10} {'Δp%':>6} | {'sock_d':>10} {'typ_d':>10} {'Δd%':>6}")
print("-" * 80)
...
for b, t in keys:
    ...
    print(f"{b:>5} {t:>6} {sp:>10.1f} {tp:>10.1f} {dp:>+5.1f}% | {sd:>10.1f} {td:>10.1f} {dd:>+5.1f}%")
EOF
